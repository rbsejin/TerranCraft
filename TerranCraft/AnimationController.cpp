...
void AnimationController::UpdateImageFrame(Image* image, ULONGLONG currentTick)
{
	uint32 sleep = image->GetSleep();
	if (sleep > 0)
	{
		image->SetSleep(sleep - 1);
		if (image->GetImageID() == BW::ImageNumber::Marine)
		{
			char buffer[256];
			sprintf_s(buffer, "ImageNumber: %d, Sleep: %d\n", (int)image->GetImageID(), sleep);
			//OutputDebugStringA(buffer);
		}
		goto LB_RETURN;
	}
	...
	case IScriptOpcode::PLAYFRAM:
	{
		...
		//image->SetFrameIndex(frameIndex);
		int8 direction = image->GetDirection();
		image->SetDirection(direction);

		printOpcode(imageNumber, "PLAYFRAM");

		//char buffer[256];
		//sprintf_s(buffer, "\tFrame: %x\n", frame);
		//OutputDebugStringA(buffer);
	}
	break;
	...
	case IScriptOpcode::WAITRAND:
	{
		...
		uint8 sleep = (rand() % 2 == 0) ? gameTicks1 : gameTicks2;
	...
	case IScriptOpcode::LOWSPRUL:
	{
		...
		//Sprite* sprite = new Sprite((BW::SpriteNumber)spriteID);
		//SpriteManager::Instance.Add(sprite);
	...
	case IScriptOpcode::END:
	{
		...
		if (parent->GetPrimaryImage() == nullptr)
		{
			//SpriteManager::Instance.Remove(parent);
			delete parent;
		}
	...
	case IScriptOpcode::FOLLOWMAINGRAPHIC:
	{
		...
		int8 frameSet = primaryImage->GetFrameSet();
		image->SetFrameSet(frameSet);
